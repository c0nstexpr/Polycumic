...
namespace polycumic::utility
{
    ...
    public:
        ...
        CPP_template(typename... Args)(requires(concepts::constructible_from<type, Args...>))
        ...
        constexpr auto operator()(First&& first, Args&&... args)
        {
            ...
            if(res) (*res)();
            else if(res.error() != validation_result::ignored)
    ...
    CPP_template(typename Getter, typename Setter)(
    ...
    CPP_template(typename Getter, typename Setter, typename Validator)(
        requires (getter_object<Getter, getter_value_t<Getter>> &&
            setter_object<Setter, getter_value_t<Getter>> &&
    ...
    template<auto GetterMem, auto SetterMem, typename T>
    constexpr auto generate_property_from_mem(T& instance)
    {
        using getter_traits = traits::member_function_pointer_traits<GetterMem>;
        using type = concepts::remove_cvref_t<typename getter_traits::result_t>;
